...
const Photos = ({ activeStep, handleStepChange }) => {
  ...
  const uploadImage = async () => {
    ...
    const itemId = JSON.parse(sessionStorage.getItem('product')).data.item[0].id;
    ...
    try {
      const response = await fetch(`${endpoint}?item_id=${encodeURIComponent(itemId)}`, {
        method: "PUT",
        body: formData,
        headers: {
  ...
  <div className="bg-[#F2F0F1] min-h-screen w-full">
    <div className="formatter">
      <div className=" bg-white rounded-lg p-10 mb-4 mt-4 ">
        {/* Header */}
        <h2 className="w-full max-w-full text-xl font-bold mb-4">Add product photos (max 5)</h2>
