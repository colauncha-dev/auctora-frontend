...
import { FaArrowDown, FaArrowUp, FaBell } from 'react-icons/fa';
import { Eye, Link, CheckCheck } from 'lucide-react';
import { useEffect, useState, useContext } from 'react';
import { NotifContext } from '../../Store/notifContex';
import Loader from '../../assets/loader2';
import PropTypes from 'prop-types';
...
const Notify = () => {
  ...
  const [loading, setLoading] = useState(false);
  const [deletingId, setDeletingId] = useState(null);
  const [activeTab, setActiveTab] = useState('inbox'); // 'inbox' or 'read'
  ...
  useEffect(() => {
    setLoading(true);
    const baseEndpoint = `${current}users/notifications?page=${currentPage}&per_page=10&order=desc`;
    const inboxEndpoint =
      activeTab === 'read' ? `${baseEndpoint}&read=true` : baseEndpoint;
    const fetchNotifications = async (endpoint, func, func2) => {
      try {
        const response = await authFetch(endpoint, {
          method: 'GET',
          ...
          <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100">
            <div className="flex space-x-2">
              <button
                ...
                className={`flex items-center space-x-2 px-3 sm:px-4 py-2 rounded-lg text-sm sm:text-base transition-all duration-200 ${
                  activeTab === 'inbox'
                    ? 'bg-gradient-to-r from-[#9f3248] to-[#de506d] text-white shadow-lg font-semibold'
                    : 'bg-white/40 text-gray-500 hover:bg-white/60 font-medium'
                }`}
              >
                <span>Inbox</span>
                {activeTab === 'inbox' && (
                ...
                {loading ? (
                  ...
                  <div className="flex flex-col items-center space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-500">
                        {activeTab === 'read'
                          ? 'Notifications you mark as read will show up here.'
              ...
              >
                <div className="p-6">
                  <div className="flex items-start justify-between relative">
                    ...
                    <div className="absolute bottom-0.5 right-1 md:relative ml-4 flex items-center space-x-2 px-4 py-2 rounded-lg transition-all duration-200">
                      {activeTab === 'read' ? (
                        <div className="relative group flex items-center">
